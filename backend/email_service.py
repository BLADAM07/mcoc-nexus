...
# SMTP Configuration (Can be customized via Environment Variables or .env)
...
def has_live_smtp() -> bool:
    """Returns True if a real SMTP password is configured."""
    return bool(SMTP_PASSWORD and len(SMTP_PASSWORD) >= 6)
...
def send_smtp_email(to_email: str, subject: str, html_body: str) -> bool:
    """Sends an HTML email via Gmail SMTP with dual SSL/TLS failover."""
    if not SMTP_PASSWORD:
        print("\n" + "="*70)
        print(f"[EMAIL SERVICE - SENDER: {SENDER_EMAIL}]")
        print(f"To: {to_email}")
        print(f"Subject: {subject}")
        print("Note: To send real live emails via Gmail, set SMTP_PASSWORD in backend/.env")
        print("="*70 + "\n")
        return True
