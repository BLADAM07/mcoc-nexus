...
# Build image lookup map of exact normalized base filenames
IMAGE_MAP: Dict[str, str] = {}
if os.path.exists(IMAGES_DIR):
    for f in os.listdir(IMAGES_DIR):
        full_path = os.path.join(IMAGES_DIR, f)
        if os.path.isfile(full_path) and f.lower().endswith(('.png', '.webp', '.jpg', '.jpeg', '.svg')):
            name_part = os.path.splitext(f)[0]
            IMAGE_MAP[normalize_key(name_part)] = f
...
def resolve_champion_image(champ_name: str, champ_class: str = "") -> str:
    ...
    if norm in ALIASES and os.path.exists(os.path.join(IMAGES_DIR, ALIASES[norm])):
    ...
    if os.path.exists(os.path.join(CLASSES_DIR, f"{class_norm}.svg")):
        return f"/images/classes/{class_norm}.svg"
        
    return "/images/Champion-star.png"

def load_all_mcoc_data() -> Dict[str, Any]:
    wb = openpyxl.load_workbook(DATASET_PATH, data_only=True)
    
    # 1. Total champions with class
    champions_list = []
    champ_by_name: Dict[str, Dict[str, Any]] = {}
    
    if "Total champion with class" in wb.sheetnames:
        sheet = wb["Total champion with class"]
        for r in range(2, sheet.max_row + 1):
            s_no = sheet.cell(r, 1).value
            name = sheet.cell(r, 2).value
            c_class = sheet.cell(r, 3).value
            if name:
                c_name = str(name).strip()
                c_cls = str(c_class).strip() if c_class else "Cosmic"
                img_url = resolve_champion_image(c_name, c_cls)
                champ_obj = {
                    "id": len(champions_list) + 1,
                    "s_no": int(s_no) if s_no and str(s_no).replace('.0','').isdigit() else len(champions_list) + 1,
                    "name": c_name,
                    "class": c_cls,
                    "image": img_url,
                    "rarities": [7, 6, 5],
                    ...
                    "tags": [],
                    "tier": "A-Tier",
                    "prestige": 14850 if len(champions_list) % 5 == 0 else 14200 + (len(champions_list) % 500)
                }
                champions_list.append(champ_obj)
                champ_by_name[normalize_key(c_name)] = champ_obj
